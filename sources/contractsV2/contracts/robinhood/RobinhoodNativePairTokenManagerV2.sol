...
import {ICreationFeeBuybackAdapter} from "../../../contracts/contracts/robinhood/interfaces/ICreationFeeBuybackAdapter.sol";
...
/// @notice Fresh V2 native-pair launch manager with paired-token dividends.
/// @dev This contract is intended for a new V2 manager proxy. Existing V1
/// launches should stay on the old manager.
contract RobinhoodNativePairTokenManagerV2 is
    OwnableUpgradeable,
    UUPSUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IReferrerRegistryV2 public referrerRegistry;
    ...
    function initialize(
        ...
        tokenIdentifier = initialTokenId == 0 ? INITIAL_TOKEN_ID : initialTokenId;
    ...
    function setReferrerRegistry(address registry) external onlyOwner {
        referrerRegistry = IReferrerRegistryV2(registry);
    }
    ...
    function _createToken(
        ...
        dividendController.registerToken(
            tokenId, tokenContractAddress, WETH_ADDRESS, excludedAccounts
        );
        ...
        returns (uint256 totalCost)
    {
        ...
        _handleFeeTransfers(tokenId, feeData, totalCost);
        emit Buy(
            msg.sender,
            tokenId,
            amount,
    ...
    function _createLp(uint256 tokenId) internal {
        ...
        IWETH9V2(WETH_ADDRESS).deposit{value: nativeAmount}();
        token.approve(address(arenaPoolDeployer), tokenAmount);
        IERC20(WETH_ADDRESS).forceApprove(address(arenaPoolDeployer), nativeAmount);
    ...
    function _handleFeeTransfers(
        ...
        if (feeData.creatorFeeRecipient != address(0) && feeData.creatorFee > 0) {
            (bool success,) = payable(feeData.creatorFeeRecipient).call{
    ...
    function _depositDividendFee(uint256 tokenId, uint256 amount) internal {
        IWETH9V2(WETH_ADDRESS).deposit{value: amount}();
        IERC20(WETH_ADDRESS).forceApprove(address(dividendController), amount);
        dividendController.deposit(tokenId, WETH_ADDRESS, amount);
