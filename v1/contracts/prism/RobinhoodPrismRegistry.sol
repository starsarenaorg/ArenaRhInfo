...
interface IPrismManagerIdentity {
    function PAIR_TOKEN() external view returns (address);
}

/// @notice Canonical allowlist of RWA numeraires and their isolated managers.
contract RobinhoodPrismRegistry is Ownable2Step {
    struct Pair {
        address manager;
        uint8 decimals;
        bool enabled;
        ...
        onlyOwner
    {
        ...
        address actual = IPrismManagerIdentity(manager).PAIR_TOKEN();
        ...
        if (decimals == 0 || decimals > 18) revert UnsupportedDecimals(decimals);
        if (_pairs[pairToken].manager == address(0)) _pairTokens.push(pairToken);
    ...
    function pairAt(uint256 index) external view returns (address pairToken, Pair memory pair) {
